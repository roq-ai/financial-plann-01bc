...
import { getFinancialGoals } from 'apiSdk/financial-goals';
import { FinancialGoalInterface } from 'interfaces/financial-goal';

import { FinancialGoalListPage as ShowCaseList } from 'pages/financial-goals';
...
function DashboardPage() {
  ...
  const financialGoalStat = useSWR<PaginatedInterface<FinancialGoalInterface>>(
    () => `/financial-goals`,
    () => getFinancialGoals({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    },
    {
      ...
      result: financialGoalStat?.data?.totalCount,
      isLoading: financialGoalStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Financial Goals
                </Text>
                <NextLink href={`financial-goals`} passHref legacyBehavior>
                  <Button
